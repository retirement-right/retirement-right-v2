...
def run_projection(client_data: dict) -> dict:
    ...
    # Option A: Tax brokerage DRAWS only (realized), not unrealized annual earnings
    # Brokerage growth is deferred until sold — taxing it annually overstates the burden
    combined = merge_tables(waterfall_table, ss_table, fixed_table, portfolio_table)
    for row in combined:
        base_ira = row.get('ira_distributions', 0)
        # Only add brokerage draws — these are realized and taxable
        brok_draw  = row.get('brokerage_draw', 0) or 0
        cash_draw  = row.get('cash_draw', 0) or 0
        # Unrealized brokerage/MM earnings NOT taxed annually (deferred until sold)
        taxable_additions = brok_draw + cash_draw
